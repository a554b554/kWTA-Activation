...
class SparseTransition(nn.Module):
    def __init__(self, in_planes, out_planes, sparsity=0.5, sparse_func='reg'):
...
class SparseDenseNet(nn.Module):
    def __init__(self, block, nblocks, sparse_func, sparsities, growth_rate=12, reduction=0.5, num_classes=10, in_channel=3):
    ...
    def _make_sparsedense_layers(self, block, in_planes, nblock, sparsity, sparse_func):
        layers = []
        for i in range(nblock):
